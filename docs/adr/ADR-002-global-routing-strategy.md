...
Users are expected to originate from both India-proximate and US-proximate locations. The public API must be reachable at `api.sporder.com`.
...
```
api.sporder.com  (Route 53 public hosted zone)
...
| `/shared/{env}/api-subdomain`          | `api.sporder.com`                  |
...
3. After deployment, the Route 53 NS records output by `SharedStack` (`NameServers` CFn output) must be added to the domain registrar (e.g., Route 53 domains, GoDaddy) as NS records for the `api.sporder.com` subdomain.
