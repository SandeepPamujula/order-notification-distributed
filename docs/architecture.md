...
A globally distributed, event-driven microservices system for processing orders at scale. The system spans two AWS regions (`ap-south-1` India, `us-east-1` US) with automatic failover via Route 53. The API is accessible at `api.spworks.click`.
...
sequenceDiagram
    ...
    User->>R53: POST api.spworks.click/orders
